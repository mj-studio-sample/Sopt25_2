...
class SignUpViewModel(private val userRepository: UserRepository) : ViewModel() {
    ...
    fun onClickSignUpButton() {
        ...
        viewModelScope.launch {
            val result = kotlin.runCatching {withContext(Dispatchers.IO){userRepository.signUp(id.value!!,pw.value!!)}}

            if(result.isSuccess) {
                _signUpComplete.value = Once(Unit)
            }else {
                _alertMsg.value = Once("회원가입에 실패했습니다. error : ${result.exceptionOrNull()}")
            }
